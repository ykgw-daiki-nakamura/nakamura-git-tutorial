...
- 顧客要望は、**顧客名を出さない短命ブランチ（`feat/...`）** で実装する
- たとえ初手が「その場での書き換え」でも、**別ブランチではなく `main` の中にスイッチ越しで入れる**。こうすれば上流の更新はそのまま乗り、コンフリクトが起きない
...
gitGraph
    ...
    commit id: "顧客カスタマイズ"
    checkout release/5.1
    merge feat/acme tag: "v5.1.1"
    checkout main
    merge release/5.1 id: "本流へ前進マージ"
```

1. **土台は `release/5.1`**（`v5.2` のコードを含まない保守線）
2. 顧客案件は **`release/5.1` から枝を切って開発**する（`main` からは切らない）→ **`v5.2` 混入なし**
...
`release/5.1` → `main` へ前進マージするとき、差分は 2 種類に分かれます。**汎用的な改善（バグ修正・どの顧客にも良い機能）は、迷わず `main` へ前進マージ**します。判断が要るのは**その顧客固有の振る舞い**です。ここに 2 つの案があります。
