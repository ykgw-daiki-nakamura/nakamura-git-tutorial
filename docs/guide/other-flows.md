...
::: tip この節の位置づけ
ここでは Release Flow を**戦略の紹介・比較**として扱い、「どういう考え方か」に徹します。実際にどの戦略を採用するかは、チームやプロダクトごとの規約で決めることです。
:::
...
逆に、継続デプロイの単一サービスで並行保守する版が無いなら、長命な `release` ブランチは使われないまま残ります。その場合は [GitHub Flow](./github-flow) の構造で足り、Release Flow の考え方は必要になった時点で採り入れられます。戦略の選び方は [ブランチ戦略の使い分け](./branching-strategies) を参照してください。
