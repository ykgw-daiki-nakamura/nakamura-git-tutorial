...
::: tip 本チュートリアルの規約では Rulesets を使う
GitHub は Branch protection rules の後継として **Repository Rulesets** を提供しています。上の 3 条件は Rulesets でも同じように設定でき、さらにブランチとタグの保護、bypass 権限の管理を一元化できます。本チュートリアルの [ブランチ運用の規約](/standards/branching) は Rulesets を採用しています。
:::
