...
stash は手軽ですが、**名前のない一時退避**なので、積み上がると「どれが何だったか」を見失いがちです。数時間以上の作業や、他の人と共有したい変更は、**作業用ブランチを切って WIP コミットする**ほうが安全です（`git commit -m "wip: ..."` して後で `git commit --amend` で整える）。stash は「数分〜十数分、ちょっと退避したい」ときの道具と考えると使い分けやすくなります。

---

一時退避を覚えておくと、割り込み作業に慌てず対応できます。よく使うコマンドは [コマンド早見表](./commands) にまとまっています。
