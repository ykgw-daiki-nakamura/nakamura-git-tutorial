...
同じプロダクトを **SaaS 版**（自社が運用してホスト）と **セルフホスト版**（顧客が自分の環境に導入して運用。オンプレミスや持ち込み VM など）の両方で提供するとき、「リリース」の意味が 2 つに割れます。このページは、[GitLab Flow](./other-flows#gitlab-flow) の**リリースブランチ版**を土台に、**対外バージョン（セルフホスト）と環境自動リリース（SaaS）を、単一の `main` の上で両立**させる設計をまとめます。
...
**[GitHub Release](./release#github-release) はタグに紐づく＝出荷点にだけ打つもの**です。セルフホストは版を出荷単位として顧客に見せるので必ず要ります。SaaS は、バージョン管理がプロセス上不要なら Release ではなく Deployments で「いま本番に何が出ているか」を追えば足ります。**ただし監査・変更管理・サポートの都合で SaaS 側も SemVer 管理が要る場合**は、[SaaS 側もバージョン管理が要るとき](#saas-側もバージョン管理が要るとき)で扱います。
...
- **SaaS**: タグ不要、継続デプロイ。上図は `main → staging → production` の[環境ブランチ](./other-flows#パターン-a-環境ブランチ)で環境を表しています。ただし環境をブランチで表すと、**環境間でコードがドリフトし、昇格のマージ経路が組み合わせで増えていく**という代償を伴います。ブランチを増やさず、デプロイパイプラインと GitHub Environments に「いまどの環境に何が出ているか」を持たせる構成も取れます（この場合、本番へ流す起点はブランチへの push ではなくタグになります）。
- **セルフホスト**: リリース時に `main` から `release/x.y` を切り、[リリースブランチ](./other-flows#パターン-b-リリースブランチ)として SemVer タグと GitHub Release を出す。旧版の保守は複数の `release/*` を残して行う（[複数バージョンの保守](./release-branches)）。

> [!NOTE]
> **なぜセルフホストだけ Release が要るのか**
>
...
> [!TIP]
> **モデル 1 と モデル 2 のどちらを選ぶか**
>
> 判断軸は「SaaS への機能投入をリリース周期から切り離したいか」です。切り離したいならモデル 1、SaaS とセルフホストに同じ版のコードを走らせてサポートを単純化したいならモデル 2 になります。モデル 1 では SaaS が GA より先を走るため、SaaS 固有の不具合を自社が最初に踏みます。モデル 2 では問い合わせ対応で版を特定する手間が減る代わりに、SaaS への機能投入がリリース周期に律速されます。どちらを選んでも版線は 1 本のままなので、運用しながら乗り換えられます。

## 対外バージョンを自動で出す

手動タグ運用は忘れ・ズレが起きるので、**[Conventional Commits](./release#conventional-commits-と対応している)から版を自動計算**するツールを噛ませます。

- **release-please**: `main` のコミットから次版を判定し、「リリース PR」（version bump + CHANGELOG）を自動で開く。それをマージした瞬間に**タグ + GitHub Release を自動作成**する。リリース PR を挟むので、**出荷の節目は人が決める**。
- **semantic-release**: `main` へのマージを検知して、そのままタグ・Release まで走らせる。**マージが即出荷になる**。

出荷の節目を人が握りたいなら release-please、継続デプロイと同じ速度で版を刻みたいなら semantic-release、という分かれ方です。
...
permissions:
  contents: write
  pull-requests: write
# actions は commit SHA でピン留めし # vX.Y.Z コメントを添える
```
...
多くの現場のプロセスは「**staging までは何度でもリリースしてよいが、本番には検証が完了し修正も取り込まれた版だけを流せる**」という形を取ります。これは案Aと案Bの“いいとこ取り”で、多くの現場が実際に採っている形です。
...
これは「リリーストレイン」の考え方そのものです（[複数バージョンの保守](./release-branches#リリーストレインと-lts) を参照）。staging で版を熟成させ、検証ゲートを越えた版だけを本番とセルフホストに配ります。このゲートが、SaaS 本番とセルフホスト GA に共通の“出荷点”になります。
...
1 つの修正を**すべての出荷先に届ける**のが最重要です。基本は [リリースブランチ運用](./release-branches) と同じ **upstream-first（＝ main-first）+ cherry-pick**。デュアル配布では、その配り先が **SaaS（環境昇格）とセルフホスト（`release/*` 各系列）の 2 レーン**に分かれるのが特徴です。

```mermaid
gitGraph
    commit id: "脆弱性修正(upstream-first)" type: HIGHLIGHT
    branch "release/1.6"
...
- upstream-first を守れば取り込み漏れは構造的に起きません（逆順にすると次版で再発。仕組みの詳細は [リリースブランチ運用](./release-branches) を参照）。

## 決めておくこと

1. **サポート版マトリクス**: いくつ前まで（N-1 / N-2）セキュリティ backport するか。無限には支えられないので明文化する。
2. **エディション差分の出し分け**: SaaS 限定／セルフホスト限定の機能を**ブランチで分けない**。単一 `main` のまま**ビルドフラグ・設定・フィーチャーフラグ**で切り替える（[顧客別カスタマイズ](/standards/customization) の考え方）。ブランチで分けると保守が破綻します。なお、フィーチャーフラグ（feature flag）は[現時点では利用を見送り検討中](/standards/versioning#feature-flag-運用)のため、当面はビルドフラグ・設定で切り替える。
3. **成果物の中身**: セルフホストの Release には、顧客が導入する**インストーラ / コンテナイメージ / Helm chart** 等の実体を添付する。

> [!NOTE]
> **これは実在するモデル**
>
> GitLab 自身がこの形です。**GitLab.com（SaaS）** は auto-deploy ブランチで継続デプロイし、**self-managed（セルフホスト相当）** は毎月の stable ブランチから版付きリリースを出して backport で保守しています。実際に運用されている構成です。

## 関連ページ

- [GitLab Flow](./other-flows#gitlab-flow) — 環境ブランチ／リリースブランチの基礎
- [リリースとバージョン管理](./release) — タグ・SemVer・GitHub Release の基本
- [複数バージョンの保守（リリースブランチ運用）](./release-branches) — 旧版保守と cherry-pick の実際
- [顧客別カスタマイズ（開発規約）](/standards/customization) — エディション差分の扱い
