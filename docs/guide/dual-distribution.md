...
1 つの修正を**すべての出荷先に届ける**のが最重要です。基本は [リリースブランチ運用](./release-branches) と同じ **main-first + cherry-pick**。デュアル配布では、その配り先が **SaaS（環境昇格）とセルフホスト（`release/*` 各系列）の 2 レーン**に分かれるのが特徴です。
...
- main-first を守れば取り込み漏れは構造的に起きません（逆順にすると次版で再発。仕組みの詳細は [リリースブランチ運用](./release-branches) を参照）。
