...
::: tip このサイト自体のデプロイ
このチュートリアルサイトは、`main` に push されるたびに GitHub Actions が自動で GitHub Pages へデプロイする**継続デプロイ型**です（[deploy.yml](https://github.com/ykgw-daiki-nakamura/nakamura-git-tutorial/blob/main/.github/workflows/deploy.yml)）。つまりこのサイトでは、タグや Release は**デプロイのトリガーではなく「出荷点の印」**として使います。プロダクトによっては、次章で触れるように**タグや Release をデプロイのトリガー**にする構成もあります。
:::
...
このリポジトリで推奨している [Conventional Commits](https://www.conventionalcommits.org/ja/)（`type(scope): 要約`）は、そのまま SemVer に対応します。**普段書いているコミット種別が、次に上げるべき桁を教えてくれます。**
...
継続デプロイ中心の Web サービスなら、多くの場合 release ブランチは不要で、hotfix も「`main` を直して再デプロイ」で済みます。**複数バージョンを並行して保守する**、**出荷後に安定化期間を設ける**——そうした「出荷済みを直し続ける」必要が出てから導入すれば十分です。`develop` / `release` を常設する **Git Flow**、`main` + `release/*` の **GitLab Flow** など、より形式化した運用もあります。有名 OSS が実際に採る**長命なリリースブランチ**運用は [複数バージョンの保守（リリースブランチ）](./release-branches) で詳しく扱います。
:::

## CI と組み合わせる（例）

タグや Release を**デプロイのトリガー**にすると、「タグを打つ＝出荷」を自動化できます。GitHub Actions での例です（このリポジトリには組み込みません。あくまで構成例です）。
...
出荷済みの複数バージョンを並行して長期保守する運用は、次の [複数バージョンの保守（リリースブランチ運用）](./release-branches) で詳しく扱います。さらに顧客ごとのカスタマイズや、特定バージョンを土台にした案件の進め方は、開発規約の [顧客別カスタマイズ](/standards/customization)・[バージョン運用](/standards/versioning) を参照してください。
