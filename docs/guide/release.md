...
出荷済みの複数バージョンを並行して長期保守する運用は、次の [複数バージョンの保守（リリースブランチ運用）](./release-branches) で詳しく扱います。さらに顧客ごとのカスタマイズや、特定バージョンを土台にした案件の進め方は、発展トピックの [顧客カスタマイズとバージョン運用](./customization) を参照してください。
