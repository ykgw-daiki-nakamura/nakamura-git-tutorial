...
::: tip 発展・任意トピック
この章は**発展・任意**の内容です。本教材が標準とする**マージ基調のフロー**（作業ブランチを `merge` で取り込み、GitHub の「Merge pull request」でマージする）では、`rebase` は必須ではありません。履歴の仕組みに**興味があれば**読んでください。読み飛ばしても以降の章は問題なく進められます。
:::
...
ここまでが rebase の基本です。本教材の標準フロー（マージ基調）に戻る場合は、[コンフリクト解決](./conflicts) の次にあたる [git stash で一時退避](./stash) へ進んでください。
