...
**[GitHub Flow](./github-flow) が向きます**。`main` にマージしたら即デプロイ、という単線で回せるためです。常設ブランチは `main` だけで、短命ブランチと PR レビューがあれば足ります。ただし「マージ＝本番反映」なので、出荷したバージョンを後から特定する手がかりは残りません。監査や変更管理で「どの版が本番に出ているか」を追う必要があるなら、デプロイの起点を `main` への push ではなく**タグ / GitHub Release** に置く構成も検討します。
