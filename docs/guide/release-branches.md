...
::: tip このチュートリアルサイトの場合
このサイトは `main` push で GitHub Pages へ継続デプロイする**単一線**の構成です。よって長命なリリースブランチは**不要**で、タグ・Release は [リリースとバージョン管理](./release) で学んだとおり「出荷点の印」として使えば十分です。パターンAは「そういう運用が必要なプロダクトに出会ったとき」の引き出しとして知っておく、という位置づけです。
:::
