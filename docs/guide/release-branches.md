...
> **このページの位置づけ**
>
> これは「継続デプロイの単一サービス」には**基本的に不要**な、やや進んだトピックです。まず前章を読んでから、「複数バージョンを並行してサポートする必要が出てきたら」戻ってくれば十分です。
...
前章で扱った `release/1.2` は、**1 本の保守線**を切って hotfix を当てる入口でした。パターンAは、これを**複数系列に一般化**したものです。
...
> **なぜ「main-first」なのか**
>
> 逆順（先にリリースブランチだけ直す）だと、**`main` への取り込みを忘れて次のリリースで同じバグが復活**します。これは前章の hotfix でも触れた最頻出の事故です。**「main に入れてから各系列へ配る」** を徹底すれば、取りこぼしが構造的に起きません。修正が main に無い状態を作らないのが要点です。
...
| --- | --- | --- |
| **Kubernetes** | `release-1.29` | minor ごとにリリースブランチを切り、`v1.29.x` をその上で tag。修正は main→リリースブランチへ cherry-pick |
| **Node.js** | `v20.x`（+ `v20.x-staging`） | メジャー系列ごと。偶数メジャーを **LTS** として長期保守 |
| **PostgreSQL** | `REL_16_STABLE` | メジャーごとの安定ブランチに back-patch。5 年サポート |
| **Linux（stable）** | `linux-6.6.y` | 安定版メンテナが修正だけを取り込む。mainline とは別管理 |
| **Chromium / Firefox** | マイルストーンブランチ（`M120` 等） | 時期でブランチを切る「リリーストレイン」型 |
| **GitLab** | `16-7-stable` | 月次リリース。セキュリティ修正を各 stable ブランチへ back-patch |

いずれも、タグは各ブランチの上で打たれ、ブランチはサポート期間中ずっと生き続けます。リリースを作るための使い捨てブランチではなく、**バージョン系列そのものを表す住所**として機能します。
...
いずれも **「出荷した線を、次の開発とは切り離して維持する」** ための仕組みです。[GitHub Flow](./github-flow) と対比すると、違いがつかみやすくなります。

## いつ導入し、いつ導入しないか

有効な運用ですが、**維持コストも大きくなります**（複数ブランチへの cherry-pick、各系列の CI、サポート期限の管理）。導入判断の目安は次のとおりです。
...
> **継続デプロイ型サイトの場合**
>
> `main` への push で継続デプロイする**単一線**の構成で、出荷済みの版に後から手を入れる必要がないなら、長命なリリースブランチは作らずに済みます。タグ・Release は前章で学んだとおり「出荷点の印」として使えば足ります。パターンAは「そういう運用が必要なプロダクトに出会ったとき」の引き出しとして知っておく、という位置づけです。

## まとめ

- **長命なリリースブランチ**は、**複数の出荷済みバージョンを並行保守する**ための仕組み（＝パターンA）
- 鉄則は **main-first**——修正はまず `main`、そこから各系列へ **cherry-pick**。取りこぼしを構造的に防ぐ
- Kubernetes・Node.js・PostgreSQL など大規模 OSS の標準運用。タグは**ブランチの上**で打ち、ブランチは**サポート期間中ずっと**生きる
