...
いずれも、タグはリリースブランチの上で打たれ、ブランチはサポート期間中ずっと生き続けます。リリースを作るための使い捨てブランチではなく、**バージョン系列そのものを表す住所**として機能します。
...
::: tip 継続デプロイ型サイトの場合
`main` への push で継続デプロイする**単一線**の構成で、出荷済みの版に後から手を入れる必要がないなら、長命なリリースブランチは作らずに済みます。タグ・Release は [リリースとバージョン管理](./release) で学んだとおり「出荷点の印」として使えば足ります。パターンAは「そういう運用が必要なプロダクトに出会ったとき」の引き出しとして知っておく、という位置づけです。
:::
