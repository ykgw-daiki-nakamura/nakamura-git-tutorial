...
共有リポジトリでの **Actions の有効化** と、`main` の **Branch protection rules**（CI が緑でないとマージ不可、など）の設定は**オーナーが行います**（[実習の進め方](./) のオーナー向けセットアップ参照）。参加者は、用意された CI が PR で動く様子を確認します。
:::
...
::: warning CI がうまく動かないとき

- **チェックが 1 つも出ない** … フォーク直後は Actions が無効なことがあります。オーナーに **Actions タブでの有効化**を依頼してください（[実習の進め方](./) 参照）。
- **PR の base がおかしい** … PR の向き先（base）は**共有リポジトリの `main`** です。自分の fork や別ブランチに向いていないか、PR 画面上部の base を確認しましょう。
- **赤いままマージしようとする** … ステップ 2 でわざと失敗させた状態のまま進めないこと。`git revert` で緑に戻してからマージします（ブランチ保護があると赤い状態ではマージできません）。
