...
> [!NOTE]
> **本ページの規約は将来の運用開始に向けたもの**
>
> ここに定めるリリースフローとデプロイパイプラインは、**将来的に利用する予定であり、直近では利用しない。** 内容が未確定という意味ではなく（検討中のページは 🚧 で示す）、規約としては定めたうえで、適用が始まるのは運用を開始する時点からである。
...
- **リリースノートの見出し**: 自動生成されるノートの各行は、当該リリースに含まれる**マージ済み PR のタイトル**である。したがって PR タイトルの品質がそのままリリースノートの品質になる。PR タイトルは Conventional Commits に準拠させ（[PR タイトル規約](./pr-title)）、type をラベルへ、要約を見出し行へ対応させる。リリース責任者による公開前の編集は、この自動生成結果の補正に留める。
