...
**RC タグ自体は省略しない**。省略すると次の 3 つが同時に崩れる。

- 昇格元となる RC 成果物が存在せず、GA ワークフローが昇格すべきイメージ・資産を持たない
- 「GA タグと最終 RC タグが同一コミットを指すこと」の検証（後述「GA 昇格規約」）が参照先を失う
...
- **リリースノートの見出し**: 自動生成されるノートの各行は、当該リリースに含まれる**マージ済み PR のタイトル**である。したがって PR タイトルの品質がそのままリリースノートの品質になる。PR タイトルは Conventional Commits に準拠させ（[ブランチ運用の PR タイトル規約](./branching#pr-タイトル規約)）、type をラベルへ、要約を見出し行へ対応させる。リリース責任者による公開前の編集は、この自動生成結果の補正に留める。
...
- 図の左半分にある `main` への PR マージ時の CI ビルドは、`dev` 環境の動作確認だけに用いる。その成果物は GitHub Release に紐づかず、staging / production へも配布物へも昇格しない。「1 回のみ」の規定が対象とするのは出荷成果物であり、`dev` 向けビルドは含まない。
