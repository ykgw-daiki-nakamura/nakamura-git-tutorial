...
本ページはホットフィックス手順と、ロールバック / ロールフォワード方針を扱う。全体像は[概要](./)を参照。
...
1. **Feature flag による縮退（第 0 選択）**: 問題の機能をフラグ OFF で切り離せる場合、デプロイを伴わない最速・最小リスクの手段として最優先で検討する（feature flag は[現時点では利用を見送り検討中](./versioning#feature-flag-運用)のため、この選択肢も導入後に有効になる）。
