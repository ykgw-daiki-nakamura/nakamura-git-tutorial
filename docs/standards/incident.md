...
3. パッチバージョンを 1 つ上げ（`vX.Y.Z` → `vX.Y.Z+1`）、RC → 検証 → GA の手順を経て出荷する。緊急度が高い場合、リリース責任者の判断で **staging での受け入れ検証（soak）を省略**し、`vX.Y.Z+1-rc.1` タグを打った直後に同一コミットへ `vX.Y.Z+1` タグを付与できる（事後検証を必須とする）。省略できるのは検証であって **RC タグそのものは省略しない**（理由は[リリースとデプロイ](./release#緊急時の短縮経路)）。
