...
3. パッチバージョン（`Z`）を上げ、RC → 検証 → GA の手順を経て出荷する。緊急度が高い場合、リリース責任者の判断で **staging での受け入れ検証（soak）を省略**し、`vX.Y.Z+1-rc.1` タグを打った直後に同一コミットへ `vX.Y.Z+1` タグを付与できる（事後検証を必須とする）。省略できるのは検証であって **RC タグそのものは省略しない**。省略すると昇格元の成果物が存在せず、[リリースとデプロイ](./release)の「GA 昇格規約（再ビルドの禁止）」が定める同一コミット検証・ダイジェスト検証が通らないためである。
