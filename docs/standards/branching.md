...
- **例外**: 該当コードが `main` に存在しない（リファクタリング・削除済みで、そのバグが `main` には存在しない）場合に限り、`release/*` への直接 PR を認める。この場合は cherry-pick 元が存在しないため、PR 本文に「`main` に該当コードが無い理由」と「`main` 側で同じ不具合が再発しないことの根拠」を明記し、リリース責任者の承認を必須とする。upstream first の目的は `main` への取り込み漏れを防ぐことであり、取り込むべきコードが `main` に無いケースはその目的の対象外である。
