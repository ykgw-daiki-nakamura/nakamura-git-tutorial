...
2. 許可する type の一覧は設定ファイル（本リポジトリでは `.claude/checks.json` の `commit.conventional.types`）を単一の情報源とする。CI の検証スクリプトはそこを読み、type を追加・削除するときも設定ファイルだけを変更する。本規約は一覧を持たない（二重管理を避けるため）。
