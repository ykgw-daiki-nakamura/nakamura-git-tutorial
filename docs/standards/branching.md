...
本ページは開発規約の **2 章（ブランチ体系）** と **3 章（ブランチプロテクションとマージルール）** を扱う。全体像は[概要](./)を参照。
...
- 組織管理者を含め、bypass list への恒常的な登録は行わない。緊急時は bypass ではなく、規約で定めた緊急手順（[障害対応](./incident)の 6 章: RC 省略の緊急パッチ）で対応する。
