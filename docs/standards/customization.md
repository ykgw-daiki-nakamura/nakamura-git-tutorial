...
- ただし SemVer 仕様ではビルドメタデータ（`+` 以降）は**バージョンの優劣判定で無視される**ため、`1.2.0+custA.5` と `1.2.0+custA.6` はツールから見て同一バージョンである。`+custA.N` はコアとの対応を人が読み取るための識別子であり、下流リリースの新旧判定には使わない。順序が必要な場面では下流リポジトリのタグ（`custA/N`）を用いる。
