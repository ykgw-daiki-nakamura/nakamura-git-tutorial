...
- 下流リポジトリにも本規約のブランチ運用（GitHub Flow + `release/*`、PR 必須、Rulesets）を適用する。
...
2. 拡張ポイントが揃ったら、Acme 専用の下流リポジトリ（`product-ext-acme`、コアの downstream）を作成し、各フックの Acme 実装と契約テストを配置する。このリポジトリにも本規約のブランチ運用（GitHub Flow + `release/*` + PR + Rulesets）を適用する。
...
- コアイメージ（`@sha256:abc…`）をベースに Acme 拡張を重ねるだけで、**コアは再ビルドしない**。前掲の GA 昇格規約（再ビルドの禁止）を維持するため。
