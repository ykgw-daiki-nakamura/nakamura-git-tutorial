...
ビルド済み成果物のすり替えに対しては、タグ起点のデプロイと再ビルドの禁止で対処する（[リリースとデプロイ](./release)。この対処は将来の運用開始に向けたものであり、直近では利用しない）。成果物の来歴（SBOM＝構成部品の一覧、provenance＝どこで何から作られたかの証明、および署名）に関する規約は今後定める。
