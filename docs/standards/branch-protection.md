...
- **Required approvals**: 指定した人数の Approve が付くまでマージを拒否する。PR の作成者自身は承認できず、この人数には数えられない。
...
**GitHub App は GitHub Actions とは別物である。** Actions がワークフローを動かす実行基盤であるのに対し、GitHub App は組織に作成してリポジトリにインストールする**独立したアクター**（bot として振る舞うアカウント）を指す。ワークフローの中から App のトークンを借りて操作すると、その操作は App の名前で記録される。ここで言う「リリース用 GitHub App」は、リリース作業のためだけに用意したこの App である。

自動化にわざわざ App を挟むのは、**ワークフローが既定で持つ `GITHUB_TOKEN` では後続のワークフローが動かない**ためである。`GITHUB_TOKEN` が作成したタグ・Release は他のワークフローを起動しない（GitHub がワークフローの無限ループを防ぐために設けた仕様）。タグ push を待っているビルド・昇格ワークフローは、このトークンで打ったタグでは起動しない。

- **タグ作成に `GITHUB_TOKEN` を使わない。** App を組織に作成して対象リポジトリにインストールし、`contents: write` 権限を与える。ワークフロー内で `actions/create-github-app-token` などにより**インストールトークン**を発行し、そのトークンでタグを打つ。Ruleset の許可アクターには、この App を登録する。
- 専用の PAT でも代替できるが、個人アカウントに紐づかない・権限を絞れる・監査ログに App 名で残る点で App を推奨する。
