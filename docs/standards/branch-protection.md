...
ブランチとタグの保護設定、および bypass 権限の方針を定める。本ページの対象はリポジトリ管理者であり、日々の開発で従う規約は[ブランチ運用](./branching)と[マージルールと PR タイトル規約](./merge-rules)に定める。
...
- `release/vX.Y` とタグ `v*` を用いる[リリースとデプロイ](./release)の運用は将来の運用開始に向けたものであり、直近では利用しない。本節と次節の保護設定も、その運用開始に合わせて適用する。
