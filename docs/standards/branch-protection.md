...
>
> 自動化する場合は、リポジトリにインストールした GitHub App のインストールトークンや専用の PAT など、`GITHUB_TOKEN` とは別のアクターのトークンでタグを打つ必要がある。その主体も Ruleset の許可アクターに登録する。**現時点では自動化せず、リリース責任者による手動のタグ push を正とする。**
