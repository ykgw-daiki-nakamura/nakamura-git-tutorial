...
本ページは開発規約の **11 章（禁止事項）** を扱う。全体像は[概要](./)を参照。
...
| 顧客別ブランチ（`customer/*`）・顧客別フォークの作成 | パッチ展開コストが「サポート中バージョン数 × 顧客数」に爆発する。カスタマイズは Tier 1（設定/フラグ）または Tier 2（下流リポジトリ）で実現する（[顧客別カスタマイズ](./customization)の 10 章） |
...
| GA タグ・環境ごと・配布形態ごとの再ビルド | ビルドは完全な再現性を持たず、RC で検証した成果物と出荷成果物の同一性が検証不能になる。GA は再タグ付けによる昇格とする（[リリースとデプロイ](./release)の 4.4） |
...
| 本番環境への out-of-band 操作（コンソール / kubectl による直接変更・手動ロールバック） | デプロイ履歴と実環境の乖離を生む。ロールバックも必ずパイプライン経由で行う（[障害対応](./incident)の 7 章） |
