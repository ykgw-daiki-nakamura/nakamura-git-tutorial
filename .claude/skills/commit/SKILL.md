...
description: >-
  ...
  コミットせずブランチ作成を促す。type 一覧は .github/conventions.json、保護ブランチ名は
  checks.json に該当キーがあれば読む。
...
- 差分から **type** を選ぶ: `feat` `fix` `docs` `chore` `ci` `build` `refactor` `test` `perf`
  `style` `revert`。`.github/conventions.json` に `commit.conventional.types` があればそれを優先
  （CI の PR タイトル検証と同一ソース）。
