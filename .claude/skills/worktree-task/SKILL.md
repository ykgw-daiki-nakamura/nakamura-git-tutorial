...
git -C <main-worktree> worktree remove <worktree>   # メイン作業ツリーから実行
...
削除対象は他の手順と同じく `<worktree>` で書く。**手順 3 で定義したとおり `<worktree>` は絶対パス**なので、
`rm -rf` がどのディレクトリで実行されても対象は 1 つに定まる。ここを `node_modules` のような相対パスに
書き換えると、実行位置しだいでメイン作業ツリーの `node_modules` を消してしまう。
