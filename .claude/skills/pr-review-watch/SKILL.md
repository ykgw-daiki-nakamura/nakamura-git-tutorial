...
description: >-
  ...
  Watch for newly created PRs, review the diff, and post the review via the Reviews API with line-anchored inline comments (bots excluded).
---

# pr-review-watch — 新規 PR 監視・自動レビュー

リポジトリに **新しく作成された PR** を自走で監視し、検知したらレビューして結果を
**Reviews API で（行単位の指摘はインラインで）投稿**する skill。ScheduleWakeup で一定間隔ごとに自分を再起動して監視を継続する。
...
- **投稿方針** — `自動投稿` か `確認してから投稿` か。既定は **自動投稿**（検知した新規 PR のレビュー結果を確認なしで投稿する）。慎重に進めたい場合は `確認してから投稿` を明示指定する。
...
**diff に含まれる行だけ**にアンカーする（diff 外の行はインライン不可）。範囲（複数行）指摘は
`start_line`＋`line` に加えて **`start_side` も指定**する（`side` と対で、通常はいずれも `RIGHT`）。
欠けると 422 の原因になる。
...
# `--input` を渡すと gh は自動的に POST になる（明示したい場合は --method POST を足す）
