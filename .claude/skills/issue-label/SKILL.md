...
- **`types`**: タイトル先頭の Conventional Commits `type`（`feat` / `fix` / `docs` / `refactor` / `perf`）
  → 主ラベル（`enhancement` / `bug` / `documentation`）。**表に無い type（`chore` / `ci` / `build` /
  `test` / `style` / `revert`）は主ラベルを付けず、領域ラベルに委ねる**。
...
- 付けるラベルと**根拠を1行ずつ**提示する（例: `bug ← type=fix`、`harness ← 本文に .claude/ を含む`）。
...
```bash
gh issue edit <N> --add-label "bug" --add-label "harness"
```
