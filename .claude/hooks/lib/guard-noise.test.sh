...
run "main 上で -- 終端付きの削除も許可"           guard-branch.sh 0 "$G -C $TD $P origin --delete -- chore/foo"
# 引数の切り出しは「最初の push 部分文字列」ではなく `git [-C dir] push` にマッチさせる
run "前段コマンドに push の語があっても削除は許可" guard-branch.sh 0 "echo push && $G -C $TD $P origin --delete chore/foo"
mkdir -p "$TD/push-sub"
