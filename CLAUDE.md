...
- **ci.yml**（PR 時）: `build`（VitePress ビルド）/ `lint`（markdownlint + textlint + 強調の描画）/ `config-check`（設定↔実体の整合）/ `test-hooks`（guard フック回帰テスト）/ `dependency-review`（PR が追加・更新する依存の脆弱性検査）/ `audit`（lockfile 全体の脆弱性検査）。
