...
読者をどこへ運ぶかは、**サイドバー・`docFooter` の prev/next・ローカル全文検索**がすでに担っている。本文に足すページ間リンクはそれと重複したまま、ページの追加・改名・見出し変更のたびに追随コストだけを残す。次を守る。

- **末尾に「関連ページ」「次のステップ」のようなリンク一覧の節を作らない。** ページを増やすたびに既存ページを直して回ることになり、放置すれば古くなる。
- **同じページへのリンクは 1 ページ内で 1 回だけにする**（初出のみ）。2 回目以降は平文で書く。宛先が変わったときに直す箇所が 1 つで済む。
- 見出しアンカー（`#…`）付きのリンクは、**見出しを直すと壊れる**（`check:anchors` が検知する）。ページ内の位置まで示す必要があるときに限って使う。
- リンクを削るぶん、**そのページだけで意味が通る書き方**にする。他ページの内容を要約して持ち込むのではなく、同じ事実を 2 か所に書かないまま文章を成立させる。
...
- 明示的に呼ぶ `npm run <script>` は影響を受けない。一時的にライフサイクルスクリプトを走らせたいときは、**CLI フラグでこの設定を打ち消す**（`npm install --ignore-scripts=false` / `npm rebuild --ignore-scripts=false <pkg>`）。`npm rebuild` も `.npmrc` を読むため、**フラグ無しの `npm rebuild <pkg>` は再ビルド自体は行うが、ライフサイクルスクリプトは走らない**。
