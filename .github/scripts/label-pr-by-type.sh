...
# 対応表は .github/conventions.json の labels.types（issue-label skill と単一ソース）。
#
# - 対応表（labels.types）に無い type はスキップして成功終了。
...
# conventions.json の labels.types で type→label を解決
label=""
if [ -f "$conventions" ] && command -v jq >/dev/null 2>&1; then
  label=$(jq -r --arg t "$type" '(.labels.types // {})[$t] // empty' "$conventions" 2>/dev/null || true)
fi
if [ -z "$label" ]; then
